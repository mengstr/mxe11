...
loop0:
	stat(dot, &x);
	if((file = open(dotdot,0)) < 0) goto loop2; 
loop1:
	if((n = read(file,&y,16)) < 16) goto loop2;

	if(y.jnum != x.inum)goto loop1;
	close(file);
	if(y.jnum == 1)
	{
		 if(ckroot() == 0) goto loop2;
	}
	if (cat() == 0) goto loop2;
	chdir(dotdot);
	goto loop0;
loop2:
	if(off > 0)
	{
		for(n=off;n>0;n--) name[n]=name[n-1];
		name[0]='/';
		off++;
	}
	else
	...
	name[off]=0;
	chdir(name);
	if(off > 1) name[off-1]=0;		
}

ckroot() {
	int i, n;

	if((n = stat(y.name,&x)) < 0) return(0);	
	i = x.devn;
	if((n = chdir(root)) < 0) return(0);
	if((file = open(root,0)) < 0) return(0);
loop:
	if((n = read(file,&y,16)) < 16) return(0);
